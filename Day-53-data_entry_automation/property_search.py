...
class PropertySearch:
    ...
    def get_prices(self):
        ...
        price_list = [price.get_text() for price in self.soup.find_all("div", class_="list-card-price")]
        return price_list

    def get_addresses(self):
        """gets the addresses in the listings"""
        address_list = [address.get_text() for address in self.soup.find_all("address", class_="list-card-addr")]
        return address_list

    def get_links(self):
        """gets the links in the listings"""
        link_list = [link.get("href") for link in self.soup.find_all("a", class_="list-card-img")]
        return link_list
