...
class QuizInterface:
    ...
    def __init__(self, quiz: QuizBrain):
        ...
        self.question_text = self.canvas.create_text(
            150, 125, fill=THEME_COLOR, text="Question Text", font=FONT, width=280
        )
        self.canvas.grid(row=1, column=0, columnspan=2, pady=50)

        correct_image = PhotoImage(file="images/true.png")
        self.correct_button = Button(
            image=correct_image, highlightthickness=0, command=self.tick_clicked
        )
        self.correct_button.grid(row=2, column=0)

        incorrect_image = PhotoImage(file="images/false.png")
        self.incorrect_button = Button(
            image=incorrect_image, highlightthickness=0, command=self.cross_clicked
        )
